...
5. No GitHub, crie uma tag igual à versão, por exemplo `v0.10.0-alpha`.
...
```bash
git add .
git commit -m "release: v0.10.0-alpha"
git push
...
```text
AlphaQuestEditor-v0.10.0-alpha-Windows-x64.exe
AlphaQuestEditor-v0.10.0-alpha-Linux-x64
...
O build local também gera `AlphaQuestEditor-v0.10.0-alpha-Source.zip` e `SHA256SUMS.txt` para uso manual.
